...
namespace nbxcOCM
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            addressTb.Text = nbxcOCM.Properties.Settings.Default.savedAddress;
            cpuMiningCheck.Checked = nbxcOCM.Properties.Settings.Default.cpuCb;
            nvidiaMiningCheck.Checked = nbxcOCM.Properties.Settings.Default.nvCb;
            amdMiningCheck.Checked = nbxcOCM.Properties.Settings.Default.amdCb;
            gpuMiningCheck.Checked = nbxcOCM.Properties.Settings.Default.gpuCb;
            cpuUsageNum.Value = nbxcOCM.Properties.Settings.Default.cpuPerc;
            hardwareCb.SelectedIndex = nbxcOCM.Properties.Settings.Default.hwType;
            refreshTimeNum.Value = nbxcOCM.Properties.Settings.Default.statsSecs;
            writeLogCheck.Checked = nbxcOCM.Properties.Settings.Default.logCb;
            showCLICheck.Checked = nbxcOCM.Properties.Settings.Default.cliCb;
            minerSelectionCb.SelectedIndex = nbxcOCM.Properties.Settings.Default.minerPreference;
        ...
        private async void writePoolsToList(object sender, DownloadStringCompletedEventArgs e)
        {
            ...
            foreach (string[] poolInfo in poolInfoList)
            {
                if (poolInfo[0] != "" && poolInfo[1] != "")
                {
                    ...
                    entryElement.selectedCb.Checked = nbxcOCM.Properties.Settings.Default.savedPools.Contains(poolInfo[2]);
        ...
        private void AnalyzePoolsByCondition(object sender, EventArgs e)
        {
            ...
            nbxcOCM.Properties.Settings.Default.poolPreference = selectionModeCb.SelectedIndex;
        }
        ...
        private void payoutBtn_Click(object sender, EventArgs e)
        {
            ...
            foreach (PoolEntry pe in poolListPanel.Controls)
            {
                if(pe.getMiningAddress() == pooladd && addressTb.Text.StartsWith("XCR"))
                {
        ...
        private void startBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!addressTb.Text.StartsWith("XCR") && addressTb.Text.Length != 98)
                {
                    MessageBox.Show("Please check your Nibble receiving address.");
                    return;
                }
                ...
                if (minerSelectionCb.Text == "XMRig")
                {
                    ...
                    if (cpuMiningCheck.Checked)
                    {
                        ...
                        cpuMinerInfo.FileName = nbxcOCM.Properties.Settings.Default.xmrigCPUpath; // @"miners\xmrig\xmrig.exe";
                        ...
                        catch (Exception excep)
                        {
                            ...
                            if (location != "")
                            {
                                ...
                                nbxcOCM.Properties.Settings.Default.xmrigCPUpath = location;
                                miner.Start();
                            }
                            else
                            {
                    ...
                    if (amdMiningCheck.Checked)
                    {
                        ...
                        amdMinerInfo.FileName = nbxcOCM.Properties.Settings.Default.xmrigAMDpath; //@"miners\xmrigamd\xmrig-amd.exe";
                        ...
                        catch (Exception excep)
                        {
                            ...
                            {
                                miner.StartInfo.FileName = location;
                                nbxcOCM.Properties.Settings.Default.xmrigAMDpath = location;
                                miner.Start();
                            }
                            else
                            {
                    ...
                    if (nvidiaMiningCheck.Checked)
                    {
                        ...
                        nvidiaMinerInfo.FileName = nbxcOCM.Properties.Settings.Default.xmrigNVpath; //@"miners\xmrignvidia\xmrig-nvidia.exe";
                        ...
                        catch (Exception excep)
                        {
                            ...
                            {
                                miner.StartInfo.FileName = location;
                                nbxcOCM.Properties.Settings.Default.xmrigNVpath = location;
                                miner.Start();
                            }
                            else
                            {
                ...
                else if (minerSelectionCb.Text == "XMR-stak")
                {
                    ...
                    arguments += " --currency cryptonight_conceal";
                    arguments += " -i 6777 -r nbxcocm";
                    ...
                    minerInfo.FileName = nbxcOCM.Properties.Settings.Default.xmrstakpath; //@"miners\xmr-stak\xmr-stak.exe";
                    ...
                    catch (Exception excep)
                    {
                        ...
                        {
                            miner.StartInfo.FileName = location;
                            nbxcOCM.Properties.Settings.Default.xmrstakpath = location;
                            miner.Start();
                        }
                        else
                        {
                ...
                nbxcOCM.Properties.Settings.Default.Save();
        ...
        {
            MessageBox.Show("With the CPU/GPU checkboxes you can (de)select which hardware xmrig/xmr-stak are supposed to mine on.\nPlease also select your hardware range to mine on certain ports with suitable difficulties.\nMine to address tells the pool to payout the mined NBXC to the given address in the textbox.\nAdvanced options are the possibility to enable logs by the miners and to view their CLIs. More are currently hidden which might change with the next releases. If you want to change specific tuning settings, please use the config files created by the miners in their folders.", "Settings explained", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        private void linkLabel3_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            MessageBox.Show("This One Click Miner for Nibble programmed by Encrypted Unicorn for Turtlecoin & modified by The Nibble Developers is a graphical interface for mining NBXC on pools in the network using xmrig by psychocrypt or xmr-stak by fireice-uk. You can find the ocm's source code on TurtleCoin's GitHub and source codes for the bundled miners on their corresponding GitHub pages. Not related to xmr-stak or xmrig.\nNibble-OCM Version " + Program.version, "About", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (PoolEntry pe in poolListPanel.Controls)
                {
                    if (pe.getMiningAddress() == connectedServerLbl.Text.Split(':')[1].Trim() && addressTb.Text.StartsWith("XCR"))
                    {
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            nbxcOCM.Properties.Settings.Default.savedAddress = addressTb.Text;
        }
        ...
        private void numericUpDown1_ValueChanged(object sender, EventArgs e)
        {
            ...
            nbxcOCM.Properties.Settings.Default.statsSecs = (int)refreshTimeNum.Value;
        }
        ...
        private void minerSelectionCb_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            nbxcOCM.Properties.Settings.Default.minerPreference = minerSelectionCb.SelectedIndex;
        }
        ...
        {
            nbxcOCM.Properties.Settings.Default.cpuCb = cpuMiningCheck.Checked;
        }

        private void gpuMiningCheck_CheckedChanged(object sender, EventArgs e)
        {
            nbxcOCM.Properties.Settings.Default.gpuCb = gpuMiningCheck.Checked;
        }

        private void amdMiningCheck_CheckedChanged(object sender, EventArgs e)
        {
            nbxcOCM.Properties.Settings.Default.amdCb = amdMiningCheck.Checked;
        }

        private void nvidiaMiningCheck_CheckedChanged(object sender, EventArgs e)
        {
            nbxcOCM.Properties.Settings.Default.nvCb = nvidiaMiningCheck.Checked;
        }

        private void hardwareCb_SelectedIndexChanged(object sender, EventArgs e)
        {
            nbxcOCM.Properties.Settings.Default.hwType = hardwareCb.SelectedIndex;
        }

        private void writeLogCheck_CheckedChanged(object sender, EventArgs e)
        {
            nbxcOCM.Properties.Settings.Default.logCb = writeLogCheck.Checked;
        }

        private void showCLICheck_CheckedChanged(object sender, EventArgs e)
        {
            nbxcOCM.Properties.Settings.Default.cliCb = showCLICheck.Checked;
        }

        private void cpuUsageNum_ValueChanged(object sender, EventArgs e)
        {
            nbxcOCM.Properties.Settings.Default.cpuPerc = (int)cpuUsageNum.Value;
        }
        ...
        private void linkLabel4_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            nbxcOCM.Properties.Settings.Default.savedPools = "";
            nbxcOCM.Properties.Settings.Default.savedAddress = "";
            nbxcOCM.Properties.Settings.Default.cpuCb = true;
            nbxcOCM.Properties.Settings.Default.nvCb = false;
            nbxcOCM.Properties.Settings.Default.amdCb = false;
            nbxcOCM.Properties.Settings.Default.gpuCb = true;
            nbxcOCM.Properties.Settings.Default.cpuPerc = 100;
            nbxcOCM.Properties.Settings.Default.hwType = 0;
            nbxcOCM.Properties.Settings.Default.statsSecs = 10;
            nbxcOCM.Properties.Settings.Default.logCb = false;
            nbxcOCM.Properties.Settings.Default.cliCb = false;
            nbxcOCM.Properties.Settings.Default.minerPreference = 0;
            nbxcOCM.Properties.Settings.Default.poolPreference = 0;
            nbxcOCM.Properties.Settings.Default.xmrigCPUpath = @"xmrig.exe";
            nbxcOCM.Properties.Settings.Default.xmrigAMDpath = @"xmrig-amd.exe";
            nbxcOCM.Properties.Settings.Default.xmrigNVpath = @"xmrig-nvidia.exe";
            nbxcOCM.Properties.Settings.Default.xmrstakpath = @"xmr-stak.exe";
            ...
            showCLICheck.Checked = nbxcOCM.Properties.Settings.Default.cliCb;
            minerSelectionCb.SelectedIndex = nbxcOCM.Properties.Settings.Default.minerPreference;
            selectionModeCb.SelectedIndex = nbxcOCM.Properties.Settings.Default.poolPreference;
        ...
        {
            System.Diagnostics.Process.Start("https://nibble-nibble.com/"); 
        }
    }
}
